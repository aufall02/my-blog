...
- `astro.config.mjs` sets canonical site URL to `https://www.ngecode.id` and enables `@astrojs/sitemap`.
- `public/robots.txt` points crawlers to `https://www.ngecode.id/sitemap-index.xml`.
...
- Keep generated URLs and metadata aligned with the configured production domain (`www.ngecode.id`).
